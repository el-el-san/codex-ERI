...
trusted_commands = [
    ...
    # 安全なテキスト処理（読み取り専用）
    ["sed", "-n", "*"],            # sed -nと任意のパターンを許可（読み取り専用）
    ["cut", "*"],                  # cutと任意の引数を許可
    ["sort", "*"],                 # sortと任意の引数を許可
    ["uniq", "*"],                 # uniqと任意の引数を許可
    
    # ファイル情報コマンド（読み取り専用）
    ["file", "*"],                 # fileと任意の引数を許可（ファイルタイプ確認）
    ["stat", "*"],                 # statと任意の引数を許可（ファイル情報表示）
    ["du", "*"],                   # duと任意の引数を許可（ディスク使用量確認）
    ["df", "*"],                   # dfと任意の引数を許可（ディスク空き容量確認）
]
```
...
**セキュリティ上の注意：**
以下のコマンドは危険なため信頼リストに追加しないでください：
- `awk` - 任意のスクリプト実行、ファイル書き込み可能
- `sed`（`-n`なし） - ファイルの直接編集が可能
- `less`/`more` - `!`でシェルコマンド実行可能
- `find`の`-exec`/`-delete`オプション付き - 任意のコマンド実行やファイル削除
