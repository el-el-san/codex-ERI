...
## このリポジトリについて
- OpenAI Rust 版 Codex CLI v0.107.0 をベースにしたクロスプラットフォームフォーク（最終同期: 2026-03-03 の upstream rust-v0.107.0）。
...
## 設定
- 既定パスは `~/.codex/config.toml`（`CODEX_HOME` で上書き可）。スキーマは upstream v0.107.0 と同じで、MCP の環境変数/HTTP ヘッダー設定も同スキーマです（環境変数の保持は `rmcp-client` 側で実装）。
