...
use crate::parallel_execution::can_execute_parallel;
...
use crate::protocol::ParallelExecutionStartEvent;
use crate::protocol::ParallelExecutionProgressEvent;
use crate::protocol::ParallelExecutionEndEvent;
...
impl Codex {
    ...
    pub async fn spawn(
        ...
        // Initialize parallel execution with config settings
        {
            use crate::parallel_execution::update_rate_limit_config;
            use crate::rate_limiter::RateLimitConfig;
            
            let rate_config = RateLimitConfig {
                max_concurrent_calls: config.parallel_execution.max_concurrent_calls,
                min_delay_ms: config.parallel_execution.min_delay_ms,
                parallel_enabled: config.parallel_execution.enabled,
                backoff_multiplier: 2.0,  // Keep default
                max_retries: config.parallel_execution.max_retries,
            };
            eprintln!("[Parallel Config] Initializing with max_concurrent_calls={}, enabled={}", 
                     rate_config.max_concurrent_calls, rate_config.parallel_enabled);
            update_rate_limit_config(rate_config).await;
        }
...
pub(crate) struct Session {
    ...
    /// Timeout for MCP tool calls in milliseconds
...
impl Session {
    ...
    pub fn turn_input_with_history(&self, extra: Vec<ResponseItem>) -> Vec<ResponseItem> {
        ...
        info!("History: {} items, {} bytes (~{} KB)", 
            history.len(), 
            history_json.len(), 
            history_json.len() / 1024);
        ...
        match sub.op {
            ...
            Op::ConfigureSession {
                ...
                sess = Some(Arc::new(Session {
                    ...
                    mcp_tool_timeout_ms: config.mcp_tool_timeout_ms,
                    notify,
...
async fn run_turn(
    ...
    let tools_size = tools_json.len();
    let input_json = serde_json::to_string(&input).unwrap_or_default();
    let input_size = input_json.len();
    
    info!("=== Token Usage Debug ===");
    info!("Tools count: {}", tools.len());
    info!("Tools JSON size: {} bytes (~{} KB)", tools_size, tools_size / 1024);
    info!("Input items count: {}", input.len());
    info!("Input JSON size: {} bytes (~{} KB)", input_size, input_size / 1024);
    info!("Total prompt size: {} bytes (~{} KB)", 
        tools_size + input_size, 
        (tools_size + input_size) / 1024);
    info!("========================");
    ...
    loop {
        ...
        info!("Turn attempt #{}, max_retries: {}", retries + 1, max_retries);
        ...
        match try_run_turn(sess, turn_diff_tracker, &sub_id, &prompt).await {
            ...
            Err(e) => {
                ...
                if error_str.contains("rate limit") || 
                   ...
                   error_str.contains("TPM") {
                    warn!("Rate limit error detected - not retrying: {e}");
                    sess.notify_background_event(
                        &sub_id,
                        format!("Rate limit reached: {e}. Please wait before retrying."),
                    )
                    .await;
                    return Err(e);
                }
                
                // Use the configured provider-specific stream retry budget for other errors.
...
/// Batch of items that can potentially be executed in parallel
#[derive(Debug)]
struct ItemBatch {
...
impl ItemBatch {
    ...
    fn add(&mut self, item: ResponseItem) {
        // Check if this item would break parallel eligibility
        if !matches!(item, ResponseItem::FunctionCall { .. }) {
            self.is_parallel_eligible = false;
        }
    ...
    fn is_ready(&self) -> bool {
        // Batch is ready if:
        // 1. We have max batch size (3 items)
        // 2. We have a non-function-call item (can't be parallelized)
        // 3. We have at least 2 items and they're all function calls
        self.items.len() >= 3 || 
        !self.is_parallel_eligible ||
        (self.items.len() >= 2 && self.is_parallel_eligible)
    }

    fn should_flush(&self, next_item: &ResponseItem) -> bool {
        // Flush if next item is not a function call
        !matches!(next_item, ResponseItem::FunctionCall { .. })
    }
    ...
    loop {
        ...
        let Some(event) = event else {
            ...
            if !batch.items.is_empty() {
                let batch_items = batch.take();
                let results = process_batch(sess, turn_diff_tracker, sub_id, batch_items).await?;
        ...
        let event = match event {
            Ok(ev) => ev,
            Err(e) => {
                // Process any remaining batch items before returning error
                if !batch.items.is_empty() {
                    let batch_items = batch.take();
                    let results = process_batch(sess, turn_diff_tracker, sub_id, batch_items).await?;
                    output.extend(results);
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                if !batch.items.is_empty() && batch.should_flush(&item) {
                    let batch_items = batch.take();
                    let results = process_batch(sess, turn_diff_tracker, sub_id, batch_items).await?;
                    output.extend(results);
                    batch = ItemBatch::new();
            ...
            ResponseEvent::Completed {
                ...
                // Process any remaining batch items before completing
                if !batch.items.is_empty() {
                    let batch_items = batch.take();
                    let results = process_batch(sess, turn_diff_tracker, sub_id, batch_items).await?;
                    output.extend(results);
...
/// Process a batch of ResponseItems, potentially in parallel
async fn process_batch(
    ...
    if items.len() == 1 {
        ...
        return Ok(vec![ProcessedResponseItem { item, response }]);
    }
    ...
    for item in items {
        let response = handle_response_item(sess, turn_diff_tracker, sub_id, item.clone()).await?;
        results.push(ProcessedResponseItem { item, response });
    }
    Ok(results)
}

/// Execute items in parallel with proper synchronization
async fn execute_parallel_items(
    sess: &Session,
    ...
    // Generate a unique group ID for this parallel execution
    let group_id = Uuid::new_v4().to_string();
    
    // Extract tool names for the start event
    let tool_names: Vec<String> = items.iter().map(|item| match item {
        ResponseItem::FunctionCall { name, .. } => name.clone(),
        ResponseItem::LocalShellCall { .. } => "shell".to_string(),
        _ => "unknown".to_string(),
    }).collect();
    ...
    // Note: We cannot use tokio::spawn here due to lifetime constraints
    // Instead, we'll use join_all to execute futures concurrently
    ...
    for (index, item) in items.into_iter().enumerate() {
        ...
        let tool_name = tool_names_clone.get(index).cloned().unwrap_or_else(|| "unknown".to_string());
        
        // Create a future for each item
        let fut = async move {
            // For parallel execution, we need a separate tracker to avoid conflicts
            let mut local_tracker = TurnDiffTracker::new();
            ...
            // Send progress event after each tool completes
            let progress_event = Event {
                ...
                msg: EventMsg::ParallelExecutionProgress(ParallelExecutionProgressEvent {
                    ...
                    completed: index + 1,  // This is approximate since they run in parallel
                    total: tool_names_clone.len(),
                    completed_tool: Some(tool_name),
                }),
            };
            sess.send_event(progress_event).await;
    ...
    // Send parallel execution end event
    let end_event = Event {
        id: sub_id.to_string(),
        ...
        _ => {
            match sess.mcp_connection_manager.parse_tool_name(&name) {
                Some((server, tool_name)) => {
                    // Use timeout from config, default to 30 seconds if not specified
                    let timeout = sess.mcp_tool_timeout_ms
