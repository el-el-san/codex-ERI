...
#[derive(Debug, Clone)]
pub(crate) struct ParallelExecutionStatus {
    pub group_id: String,
...
pub(crate) struct BottomPane<'a> {
    ...
    // Input queueing system for accepting inputs during task processing
    queued_inputs: VecDeque<String>,
    enable_input_queueing: bool,
    // Temporary buffer for accumulating input during task running
    input_buffer: String,
...
pub(crate) struct BottomPaneParams {
    ...
    pub(crate) custom_commands: Vec<codex_core::custom_command::CustomCommand>,
}
...
impl BottomPane<'_> {
    ...
    pub fn handle_key_event(&mut self, key_event: KeyEvent) -> InputResult {
        // Check if we're in task running mode with input queueing enabled
        if self.is_task_running && self.enable_input_queueing && self.status_view_active {
            // Handle input for queueing while showing status
            return self.handle_queued_input(key_event);
        }
    ...
    pub fn update_parallel_execution_status(&mut self, status: Option<ParallelExecutionStatus>) {
        self.parallel_execution_status = status;
    }
    ...
    /// Handle input during task running for queueing
    fn handle_queued_input(&mut self, key_event: KeyEvent) -> InputResult {
        ...
        match key_event {
            KeyEvent {
                ...
                // Add current buffer to queue if not empty
                if !self.input_buffer.is_empty() {
                    self.queued_inputs.push_back(self.input_buffer.clone());
    ...
    /// Update the status display to show queued input information
    fn update_queue_status(&mut self) {
        let queue_count = self.queued_inputs.len();
        let buffer_preview = if self.input_buffer.len() > 30 {
            format!("{}...", &self.input_buffer[..30])
        } else {
            self.input_buffer.clone()
        };
        
        let status_text = if queue_count > 0 {
            format!("Working... ({} queued) | Input: {}", queue_count, buffer_preview)
        } else if !self.input_buffer.is_empty() {
            format!("Working... | Input: {}", buffer_preview)
        } else {
            "Working... (type to queue additional input)".to_string()
        };
        
        if let Some(status) = &mut self.live_status {
            status.update_text(status_text.clone());
        }
    ...
    /// Process queued inputs when task completes
    pub fn process_queued_inputs(&mut self) -> Vec<String> {
        let mut inputs = Vec::new();
        while let Some(input) = self.queued_inputs.pop_front() {
            inputs.push(input);
        }
        // Also add any remaining buffer content
        if !self.input_buffer.is_empty() {
            inputs.push(self.input_buffer.clone());
            self.input_buffer.clear();
        }
        inputs
    }
...
impl WidgetRef for &BottomPane<'_> {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        // Render parallel execution status if present
        if let Some(parallel_status) = &self.parallel_execution_status {
            if y_offset < area.height {
                ...
                let progress_text = format!(
                    "[並列実行中: {}/{} 完了] {}",
                    parallel_status.completed_tools,
                    parallel_status.total_tools,
                    parallel_status.running_tools.join(" | ")
                );
                ...
                paragraph.render_ref(status_rect, buf);
                y_offset = y_offset.saturating_add(1);
            }
        }
