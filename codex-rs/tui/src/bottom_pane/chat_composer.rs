...
use super::command_popup::{CommandPopup, CommandType};
...
pub(crate) struct ChatComposer {
    ...
    has_focus: bool,
    custom_commands: Vec<codex_core::custom_command::CustomCommand>,
}
...
impl ChatComposer {
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                if let Some(cmd) = popup.selected_command() {
                    ...
                    let cmd_text = match &cmd {
                        CommandType::BuiltIn(c) => c.command(),
                        CommandType::Custom(c) => c.command(),
                    };

                    let starts_with_cmd = first_line
                        .trim_start()
                        .starts_with(&format!("/{}", cmd_text));

                    if !starts_with_cmd {
                        self.textarea.set_text(&format!("/{} ", cmd_text));
                    }
                }
                (InputResult::None, true)
            }
            KeyEvent {
                ...
                if let Some(cmd) = popup.selected_command() {
                    ...
                    match cmd {
                        CommandType::BuiltIn(c) => {
                            self.app_event_tx.send(AppEvent::DispatchCommand(*c));
                        }
                        CommandType::Custom(c) => {
                            self.app_event_tx.send(AppEvent::DispatchCustomCommand(c.clone()));
                        }
                    }
    ...
    fn sync_command_popup(&mut self) {
        ...
        match &mut self.active_popup {
            ...
            _ => {
                if input_starts_with_slash {
                    let mut command_popup = CommandPopup::new(self.custom_commands.clone());
...
mod tests {
    ...
    fn handle_paste_small_inserts_text() {
        ...
        let mut composer = ChatComposer::new(true, sender, false, Vec::new());
    ...
    fn ui_snapshots() {
        ...
        for (name, input) in test_cases {
            ...
            let mut composer = ChatComposer::new(true, sender.clone(), false, Vec::new());
