    ...
    pub(crate) fn run(&mut self, terminal: &mut tui::Tui) -> Result<()> {
        ...
        while let Ok(event) = self.app_event_rx.recv() {
            match event {
                ...
                AppEvent::DispatchCustomCommand(cmd) => {
                    ...
                    if let AppState::Chat { widget } = &mut self.app_state {
                        match cmd.command_type {
                            CustomCommandType::Prompt => {
                                ...
                                widget.submit_text_message(cmd.content);
                            }
                                ...
                                match output {
                                    Ok(output) => {
                                        ...
                                        let result = if output.status.success() {
                                            ...
                                            format!("Command '{}' failed:\n{}\n{}", cmd.name, stdout, stderr)
                                        };
                                        widget.add_diff_output(result);
                                    }
                                    Err(e) => {
                                        let msg = format!("Failed to execute command '{}': {}", cmd.name, e);
                                        widget.add_diff_output(msg);
                                    }
                                }
                            }
