...
impl ChatWidget<'_> {
    ...
    pub(crate) fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        match self.bottom_pane.handle_key_event(key_event) {
            InputResult::Submitted(text) => {
                // Check if it's a slash command that wasn't handled by the popup
                if text.starts_with('/') {
                    ...
                    if let Some(custom_cmd) = self.config.custom_commands.iter().find(|c| c.name == command) {
                        let mut cmd = custom_cmd.clone();
                        
                        // If command accepts args and args are provided, replace placeholder
                        if cmd.accepts_args && args.is_some() {
                            if let Some(placeholder) = &cmd.arg_placeholder {
                                cmd.content = cmd.content.replace(placeholder, &args.unwrap());
                            }
                        }
                        
                        self.app_event_tx.send(AppEvent::DispatchCustomCommand(cmd));
                        return;
                    }
                }
    ...
    /// Process any queued inputs when a task completes
    fn process_queued_inputs(&mut self) {
        let queued = self.bottom_pane.process_queued_inputs();
        for text in queued {
            if !text.is_empty() {
                self.submit_user_message(text.into());
            }
    ...
    pub(crate) fn handle_codex_event(&mut self, event: Event) {
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent { message }) => {
                ...
                self.content_buffer.clear();
                self.request_redraw();
                // Process any queued inputs after error
                self.process_queued_inputs();
            }
            ...
            EventMsg::ParallelExecutionStart(event) => {
                use crate::bottom_pane::ParallelExecutionStatus;
                use crate::history_cell::HistoryCell;
                use crate::text_block::TextBlock;
                use ratatui::text::Line;
                
                // Add history cell for parallel execution start
                let text = format!(
                    "[並列実行開始] {} ツールを並列処理:\n{}",
                    event.total_count,
                    event.tools.iter()
                ...
                let status = ParallelExecutionStatus {
                    ...
                    total_tools: event.total_count,
                    running_tools: event.tools,
                    completed_tools: 0,
                    failed_tools: 0,
                };
                self.bottom_pane.update_parallel_execution_status(Some(status));
                self.request_redraw();
            }
            EventMsg::ParallelExecutionProgress(event) => {
                if let Some(mut status) = self.bottom_pane.parallel_execution_status.clone() {
                    status.completed_tools = event.completed;
                    if let Some(completed_tool) = event.completed_tool {
                        status.running_tools.retain(|t| t != &completed_tool);
                    }
                    self.bottom_pane.update_parallel_execution_status(Some(status));
                    self.request_redraw();
                }
            ...
            EventMsg::ParallelExecutionEnd(event) => {
                ...
                // Add history cell for parallel execution end
                let text = format!(
                    "[並列実行完了] 成功: {}/{}, 実行時間: {}ms",
                    event.successful,
    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.show_mcp_popup {
            if let Some(ref popup) = self.mcp_popup {
                ...
                // Ensure minimum height of 3 (1 for content + 2 for borders)
                // Use saturating_sub to prevent underflow
                let max_available_height = area.height.saturating_sub(2);
                let popup_height = (required_height + 2).min(max_available_height).max(3);
