...
pub(crate) enum HistoryCell {
    ...
    /// Parallel execution group start
    ParallelExecutionGroupStart { view: TextBlock },
    
    /// Parallel execution group end
    ParallelExecutionGroupEnd { view: TextBlock },
}
...
impl HistoryCell {
    ...
    pub(crate) fn plain_lines(&self) -> Vec<Line<'static>> {
        match self {
            ...
            | HistoryCell::ParallelExecutionGroupStart { view }
            | HistoryCell::ParallelExecutionGroupEnd { view }
...
impl HistoryCell {
    pub(crate) fn new_parallel_execution_group_start(
        group_id: &str,
        tools: &[String],
    ) -> Self {
        let mut lines = vec![];
        ...
        lines.push(Line::from(vec![
            Span::styled("[並列グループ開始] ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::styled(format!("ID: {}", group_id), Style::default().fg(Color::Gray)),
        ]));
        ...
        HistoryCell::ParallelExecutionGroupStart {
            view: TextBlock { lines },
        }
    }
    
    pub(crate) fn new_parallel_execution_group_end(
        group_id: &str,
        successful: usize,
        failed: usize,
        total: usize,
        ...
        let status_str = if failed == 0 {
            format!("{}/{} 成功", successful, total)
        } else {
            format!("{}/{} 成功, {} 失敗", successful, total, failed)
        };
        
        let lines = vec![
            Line::from(vec![
                Span::styled("[並列グループ終了] ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled(format!("ID: {} ", group_id), Style::default().fg(Color::Gray)),
                Span::styled(status_str, if failed == 0 { 
                    Style::default().fg(Color::Green)
                } else {
                    Style::default().fg(Color::Red)
                }),
        ...
        HistoryCell::ParallelExecutionGroupEnd {
            view: TextBlock { lines },
        }
    }
}
