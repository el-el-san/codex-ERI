...
fn open_url(url: &str) -> Result<OpenUrlStatus, String> {
    ...
    async fn finish(mut self) -> Result<()> {
        if self.launch_browser {
            ...
            match open_url(auth_url) {
                Ok(OpenUrlStatus::Opened) => { /* opened */ }
                Ok(OpenUrlStatus::Suppressed { reason }) => {
                    println!("{reason}");
                    println!("(If it didn't open, please copy the URL above manually.)");
                }
                Err(err) => {
                    println!("Failed to launch browser: {err}");
                    println!("Please copy the URL above manually.");
                }
            }
        }
