...
- upstream 0.155.1 では HTTP 通信が `http-client` に集約され、従来の `core` / `ollama` / `login` は `reqwest` を直接依存しない
...
- upstream 0.147.0 で V8 ランタイムが `code-mode-runtime`、プロセス境界が `code-mode-host` へ分離され、0.155.1 でもこの構成を維持している
...
- upstream 0.149.0 の Android release build で、CLI 起動部の大きな async 型により rustc の
  `queries overflow the depth limit!` が発生したため、0.155.1 でも再帰上限の設定を維持する
...
- `rollout/src/writer_lock.rs`（旧`thread-store/src/local/writer_lock.rs`、0.147.0 で新設）の `file.lock()` / `file.try_lock()` も同様にAndroidで非対応
  - `lock_coordination` 内の `file.lock()` と `acquire` 内の `file.try_lock()` を `#[cfg(not(target_os = "android"))]` で除外
  - `try_acquire_for_publication` も Android ではロック確認なしで通過させる
...
- `rollout/src/maintenance.rs` の保守ロックも Android では `try_lock()` が非対応
  - Android ではファイルを作成・保持するだけのガードとして扱い、ロック取得を省略する
...
- upstream 0.155.1 の正常な構成では Android 用 3 パッケージから `v8` / `rusty_v8` へ到達しない
...
- upstream 0.155.1 の `lto = "thin"` / `codegen-units = 4` でも Android 向け release build が長時間化する場合があるため、
...
- ログイン、MCP OAuth、Termux/Android環境変数保持、TLS vendoring、
  clipboard警告抑止、AndroidのDirectツール強制を再適用
...
- upstream 0.153.0 の Vim undo/redo、リモート marketplace 対応 plugin CLI、auto recap 設定、
  TUI 履歴表示・app-server 再接続・Guardian 履歴・MCP 承認スコープ・rollout 圧縮の改善、
  非同期質問と実験的 context management を取り込み
- 0.153.1～0.153.3 の GPT-6-Astra モデルカタログ、Fast tier 表示、
  Amazon Bedrock model picker と非同期質問ガイダンスの修正まで適用
- 再適用・確認した差分:
...
- 上流アーカイブの `Cargo.lock` にある workspace package version `0.0.0` を 0.153.3 へ再生成し、
...
- Android artifact を取得し、`codex` / `codex-exec` / `codex-tui` が
  Android API 28 向け ARM aarch64 ELF で、各 `--version` が 0.153.3 を返すことを確認
  - artifact: `codex-android-aarch64-release.tar.gz`
  - SHA-256: `8dcbb9071025160f39a39fcd19e4d7138a345dd3bcb9fa799aadf05c3cdbd6c8`
- Termux 上で新しい `codex exec --ephemeral` の応答を確認後、`~/bin` の 3 バイナリを置換
  - 旧 0.152.1 バイナリ: `~/bin/.codex-backup-0.152.1-20260905-1006/`
...
- upstream 0.150.0 で追加されたタスクの `@` メンション、`/copy` 選択、スレッド自動命名、
  Markdown リンク表示、Interrupt hook などを取り込み、0.150.1 のリモート圧縮時の
  画像トークン予算修正まで反映
- 再適用・確認した差分:
  - `http-client` の vendored OpenSSL、`login` / `rmcp-client` の環境別ブラウザ起動
  - `rmcp-client` の Termux/Android 環境変数保持
  ...
  - Android の Direct ツール強制と、意図した code mode フォールバック警告の抑止
  - Android release build 用の再帰上限を維持
- 上流アーカイブの `Cargo.lock` にある workspace package version `0.0.0` を 0.150.1 へ再生成し、
...
- GitHub Actions の Android aarch64 release build、Cargo audit、CodeQL が成功
- Android artifact を取得し、`codex` / `codex-exec` / `codex-tui` が
  Android API 28 向け ARM aarch64 ELF で、各 `--version` が 0.150.1 を返すことを確認
  - artifact: `codex-android-aarch64-release.tar.gz`
  - SHA-256: `dfebad92294962937410560076770742317e3fc364a3b35585842caf65b8ca58`
